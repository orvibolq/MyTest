...
#import "HMProcessCircleView.h"


#define PullToRefresh @"下拉刷新"
#define ReleaseToRefresh @"松开刷新"
#define Isloading @"正在刷新"
...
@property (nonatomic, strong)HMProcessCircleView * processCircleView;
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.titles = [NSMutableArray arrayWithObjects:PullToRefresh,
                             ReleaseToRefresh,
                            Isloading,
                                nil];
...
- (void)layoutSubviews {
    ...
    if(hasCustomView) {
        ...
        CGFloat leftViewWidth = MAX(self.animationImageView.bounds.size.width,self.activityIndicatorView.bounds.size.width);
        ...
        CGFloat arrowX = (self.bounds.size.width / 2) - (totalMaxWidth / 2) + (leftViewWidth - self.animationImageView.bounds.size.width) / 2;
        self.animationImageView.frame = CGRectMake(arrowX,
                                      (self.bounds.size.height / 2) - (self.animationImageView.bounds.size.height / 2),
                                      self.animationImageView.bounds.size.width,
                                      self.animationImageView.bounds.size.height);
//        self.activityIndicatorView.center = self.animationImageView.center;
    }
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        [self.processCircleView setProgressValue:(-contentOffset.y/SVPullToRefreshViewHeight)];
...
- (UIImageView *)animationImageView {
    if(!_animationImageView) {
		...
		_animationImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0, (CGRectGetHeight(self.frame) - imageHeight)/2, imageHeight, imageHeight)];
        _animationImageView.backgroundColor = [UIColor clearColor];
        _animationImageView.animationImages = @[[UIImage imageNamed:@"loading_1"],[UIImage imageNamed:@"loading_2"],[UIImage imageNamed:@"loading_3"],[UIImage imageNamed:@"loading_4"]];
        _animationImageView.animationDuration = 0.3;
        _animationImageView.image = [UIImage imageNamed:@"loading_1"];
        
        self.processCircleView = [[HMProcessCircleView alloc] initWithFrame:_animationImageView.bounds];
        self.processCircleView.progressColor = [UIColor greenColor];
        self.processCircleView.progressTrackColor = [UIColor clearColor];
        self.processCircleView.progressStrokeWidth = 2.0f;
        [_animationImageView addSubview:self.processCircleView];
